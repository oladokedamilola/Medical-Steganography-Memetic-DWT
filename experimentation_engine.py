...
- Total: (available images) × 7 × 3 experiments
"""
...
class ExperimentMatrix:
    ...
    def __init__(self):
        ...
        self.color_images = self._discover_images('color')
        self.grayscale_images = self._discover_images('grayscale')
        ...
        logger.info(f"  - Color images found: {len(self.color_images)}")
        logger.info(f"  - Grayscale images found: {len(self.grayscale_images)}")
    ...
    def get_statistical_power(self) -> Dict:
        """
        Calculate statistical power of the experiment design
        """
        power_info = {
            'repetitions': self.repetitions,
            'combinations_per_condition': len(self.all_images) * len(self.text_sizes),
            'total_samples': self.total_combinations,
...
class ExperimentRunner:
    ...
    def __init__(self, output_dir: str = "data/experiment_results"):
        """
    ...
    def run_single_experiment(self, experiment: Dict) -> Optional[Dict]:
        ...
        try:
            ...
            if 'stego_image' in pipeline_result:
                ...
                # For accurate BER calculation, we need to re-extract and decrypt
                ...
                # Data metrics - now passing both original payload and decrypted text
            ...
            result = {
                ...
                'plaintext': payload,  # Store original text for later use
                'decrypted_text': decrypted_text,  # Store decrypted text for verification
            ...
            # Verify BER=0 for successful runs
            if result['performance']['success']:
                ber = data_integrity.get('ber', 1.0)
                if ber > 0:
                    logger.warning(f"Experiment {experiment['experiment_id']} reported success "
                                  f"but BER={ber:.6f}")
    ...
    def _print_final_summary(self, stats: Dict):
        ...
        if self.results:
            ...
            # Check BER=0 for successful runs
            successful_runs = [r for r in self.results if r['performance']['success']]
            non_zero_ber = [r for r in successful_runs 
                           if r['metrics']['data_integrity'].get('ber', 1) > 0]
            
            if non_zero_ber:
                print(f"\n⚠️  Warning: {len(non_zero_ber)} successful runs have BER > 0")
            else:
                print(f"\n✅ All successful runs have BER = 0")
...
def main():
    ...
    try:
        # Step 1: Initialize experiment runner
        print("\n🚀 Initializing Experiment Runner...")
        runner = ExperimentRunner(output_dir="data/experiment_results")
        
        # Step 2: Run all experiments
        print("\n🧪 Running all experiments...")
        results = runner.run_all_experiments(resume=True, max_retries=3)
